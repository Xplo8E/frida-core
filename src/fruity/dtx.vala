...
namespace Frida.Fruity {
	public sealed class DeviceInfoService : Object, AsyncInitable {
		...
		public static async DeviceInfoService open (HostChannelProvider channel_provider, Cancellable? cancellable = null)
			...
			Frida.Fruity.Debug.log ("[FRIDA-DTX] Opening DeviceInfoService\n");
			var service = new DeviceInfoService (channel_provider);

			try {
				yield service.init_async (Priority.DEFAULT, cancellable);
				Frida.Fruity.Debug.log ("[FRIDA-DTX] DeviceInfoService opened successfully\n");
			} catch (GLib.Error e) {
				Frida.Fruity.Debug.log ("[FRIDA-DTX] Error opening DeviceInfoService: %s\n", e.message);
				throw_api_error (e);
			}
	...
	public sealed class ApplicationListingService : Object, AsyncInitable {
		...
		public static async ApplicationListingService open (HostChannelProvider channel_provider, Cancellable? cancellable = null)
				throws Error, IOError {
			Frida.Fruity.Debug.log ("[FRIDA-DTX] Opening ApplicationListingService\n");
			var service = new ApplicationListingService (channel_provider);

			try {
				yield service.init_async (Priority.DEFAULT, cancellable);
				Frida.Fruity.Debug.log ("[FRIDA-DTX] ApplicationListingService opened successfully\n");
			} catch (GLib.Error e) {
				Frida.Fruity.Debug.log ("[FRIDA-DTX] Error opening ApplicationListingService: %s\n", e.message);
				throw_api_error (e);
			}
	...
	public sealed class ProcessControlService : Object, AsyncInitable {
		...
		public static async ProcessControlService open (HostChannelProvider channel_provider, Cancellable? cancellable = null)
				throws Error, IOError {
			Frida.Fruity.Debug.log ("[FRIDA-DTX] Opening ProcessControlService\n");
			var service = new ProcessControlService (channel_provider);

			try {
				yield service.init_async (Priority.DEFAULT, cancellable);
				Frida.Fruity.Debug.log ("[FRIDA-DTX] ProcessControlService opened successfully\n");
			} catch (GLib.Error e) {
				Frida.Fruity.Debug.log ("[FRIDA-DTX] Error opening ProcessControlService: %s\n", e.message);
				throw_api_error (e);
			}
	...
	public sealed class DTXConnection : Object, DTXTransport {
		...
		public DTXConnection (IOStream stream) {
			Frida.Fruity.Debug.log ("[FRIDA-DTX] Creating DTXConnection\n");
			Object (stream: stream);
		}

		construct {
			Frida.Fruity.Debug.log ("[FRIDA-DTX] Initializing DTXConnection streams\n");
			input = new DataInputStream (stream.get_input_stream ());
			input.byte_order = LITTLE_ENDIAN;
			output = stream.get_output_stream ();

			Frida.Fruity.Debug.log ("[FRIDA-DTX] Creating DTX control channel\n");
			...
			try {
				...
				Frida.Fruity.Debug.log ("[FRIDA-DTX] Published DTX capabilities\n");
			} catch (Error e) {
				assert_not_reached ();
			}

			Frida.Fruity.Debug.log ("[FRIDA-DTX] Starting fragment processing\n");
		...
		public DTXChannel make_channel (string identifier) throws Error {
			Frida.Fruity.Debug.log ("[FRIDA-DTX] Creating DTX channel for identifier: %s\n", identifier);
			check_open ();
			...
			Frida.Fruity.Debug.log ("[FRIDA-DTX] DTX channel created with code: %d\n", channel_code);
