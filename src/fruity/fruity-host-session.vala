namespace Frida {
	public sealed class FruityHostSessionBackend : Object, HostSessionBackend {
		...
		public async void start (Cancellable? cancellable) throws IOError {
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Starting FruityHostSessionBackend\n");
			yield device_monitor.start (cancellable);
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] FruityHostSessionBackend started successfully\n");
		}

		public async void stop (Cancellable? cancellable) throws IOError {
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Stopping FruityHostSessionBackend\n");
			io_cancellable.cancel ();

			yield device_monitor.stop (cancellable);

			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Closing %u providers\n", providers.size);
			foreach (var provider in providers.values) {
				provider_unavailable (provider);
				yield provider.close (cancellable);
			}
			providers.clear ();
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] FruityHostSessionBackend stopped successfully\n");
		}

		private void on_device_attached (Fruity.Device device) {
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Device attached: %s (UDID: %s)\n", device.name, device.udid);
		...
		private void on_device_detached (Fruity.Device device) {
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Device detached: %s (UDID: %s)\n", device.name, device.udid);
			FruityHostSessionProvider provider;
			if (providers.unset (device, out provider)) {
				Fruity.Debug.log ("[FRIDA-HOST-SESSION] Removing provider for detached device\n");
				provider_unavailable (provider);
	...
	public sealed class FruityHostSessionProvider : Object, HostSessionProvider, HostChannelProvider, Pairable {
		...
		public async void close (Cancellable? cancellable) throws IOError {
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Closing provider for device: %s (UDID: %s)\n", device.name, device.udid);
			yield Fruity.DTXConnection.close_all (device, cancellable);
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Provider closed successfully\n");
		}

		public async HostSession create (HostSessionOptions? options, Cancellable? cancellable) throws Error, IOError {
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Creating host session for device: %s (UDID: %s)\n", device.name, device.udid);
			if (host_session != null) {
				Fruity.Debug.log ("[FRIDA-HOST-SESSION] ERROR: Host session already exists\n");
				throw new Error.INVALID_OPERATION ("Already created");
			}

			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Initializing new host session\n");
		...
		public async void destroy (HostSession session, Cancellable? cancellable) throws Error, IOError {
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Destroying host session for device: %s\n", device.name);
			...
			host_session = null;
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Host session destroyed successfully\n");
		}
	...
	public sealed class FruityHostSession : Object, HostSession {
		...
		public async HashTable<string, Variant> query_system_parameters (Cancellable? cancellable) throws Error, IOError {
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Querying system parameters for device: %s\n", device.name);
			var server = yield try_get_remote_server (cancellable);
			if (server != null && server.flavor == REGULAR) {
				Fruity.Debug.log ("[FRIDA-HOST-SESSION] Using remote server for system parameters\n");
				try {
					var params = yield server.session.query_system_parameters (cancellable);
					Fruity.Debug.log ("[FRIDA-HOST-SESSION] System parameters retrieved from remote server\n");
					return params;
				} catch (GLib.Error e) {
					Fruity.Debug.log ("[FRIDA-HOST-SESSION] Error querying remote system parameters: %s\n", e.message);
					throw_dbus_error (e);
				}
			}

			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Using lockdown client for system parameters\n");
		...
		public async void enable_spawn_gating (Cancellable? cancellable) throws Error, IOError {
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Enabling spawn gating for device: %s\n", device.name);
			var server = yield get_remote_server (cancellable);
			try {
				yield server.session.enable_spawn_gating (cancellable);
				Fruity.Debug.log ("[FRIDA-HOST-SESSION] Spawn gating enabled successfully\n");
			} catch (GLib.Error e) {
				Fruity.Debug.log ("[FRIDA-HOST-SESSION] Error enabling spawn gating: %s\n", e.message);
				throw_dbus_error (e);
			}
		}

		public async void disable_spawn_gating (Cancellable? cancellable) throws Error, IOError {
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Disabling spawn gating for device: %s\n", device.name);
			var server = yield get_remote_server (cancellable);
			try {
				yield server.session.disable_spawn_gating (cancellable);
				Fruity.Debug.log ("[FRIDA-HOST-SESSION] Spawn gating disabled successfully\n");
			} catch (GLib.Error e) {
				Fruity.Debug.log ("[FRIDA-HOST-SESSION] Error disabling spawn gating: %s\n", e.message);
				throw_dbus_error (e);
			}
		}
		...
		public async uint spawn (string program, HostSpawnOptions options, Cancellable? cancellable) throws Error, IOError {
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Spawning program '%s' on device: %s\n", program, device.name);
			var server = yield try_get_remote_server (cancellable);
			...
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Using LLDB service for app launch\n");
			var lldb = yield start_lldb_service (cancellable);
			var process = yield lldb.launch (argv, launch_options, cancellable);
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] LLDB launched process with PID %u, state: %s\n", process.pid, process.observed_state.to_string());
			if (process.observed_state == ALREADY_RUNNING) {
				Fruity.Debug.log ("[FRIDA-HOST-SESSION] Process already running, killing and relaunching\n");
				...
				Fruity.Debug.log ("[FRIDA-HOST-SESSION] Process relaunched with PID %u\n", process.pid);
			}

			var session = new LLDBSession (lldb, process, gadget_path, device);
			add_lldb_session (session);
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] LLDB session added for PID %u\n", process.pid);
		...
		public async void kill (uint pid, Cancellable? cancellable) throws Error, IOError {
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Killing process with PID %u\n", pid);
			var lldb_session = lldb_sessions[pid];
			if (lldb_session != null) {
				Fruity.Debug.log ("[FRIDA-HOST-SESSION] Using existing LLDB session to kill PID %u\n", pid);
				yield lldb_session.kill (cancellable);
				Fruity.Debug.log ("[FRIDA-HOST-SESSION] Process %u killed via LLDB session\n", pid);
				return;
			}
		...
		public async AgentSessionId attach (uint pid, HashTable<string, Variant> options,
				Cancellable? cancellable) throws Error, IOError {
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Attempting to attach to process with PID %u\n", pid);
			var lldb_session = lldb_sessions[pid];
			if (lldb_session != null) {
				Fruity.Debug.log ("[FRIDA-HOST-SESSION] Found existing LLDB session for PID %u, querying gadget details\n", pid);
			...
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] No LLDB session for PID %u, checking for remote server\n", pid);
			var server = yield try_get_remote_server (cancellable);
			if (server != null) {
				Fruity.Debug.log ("[FRIDA-HOST-SESSION] Remote server available, attempting remote attach\n");
				try {
					return yield attach_via_remote (pid, options, server, cancellable);
				} catch (Error e) {
					Fruity.Debug.log ("[FRIDA-HOST-SESSION] Remote attach failed: %s\n", e.message);
					if (server.flavor == REGULAR)
						throw_api_error (e);
				}
			}
		...
		private async LLDB.Client start_lldb_service (Cancellable? cancellable) throws Error, IOError {
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Starting LLDB service\n");
			foreach (unowned string endpoint in DEBUGSERVER_ENDPOINT_CANDIDATES) {
				Fruity.Debug.log ("[FRIDA-HOST-SESSION] Trying to connect to debugserver endpoint: %s\n", endpoint);
				try {
					var lldb_stream = yield device.open_lockdown_service (endpoint, cancellable);
					Fruity.Debug.log ("[FRIDA-HOST-SESSION] Connected to debugserver endpoint: %s\n", endpoint);
					Fruity.Debug.log ("[FRIDA-HOST-SESSION] Opening LLDB client\n");
					return yield LLDB.Client.open (lldb_stream, cancellable);
				} catch (Error e) {
					Fruity.Debug.log ("[FRIDA-HOST-SESSION] Failed to connect to debugserver endpoint %s: %s\n", endpoint, e.message);
		...
		private async AgentSessionId attach_via_gadget (uint pid, HashTable<string, Variant> options,
			...
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Attaching via gadget to PID %u (gadget on port %u)\n", 
				pid, gadget_details.port);
			try {
				Fruity.Debug.log ("[FRIDA-HOST-SESSION] Opening TCP channel to gadget port %u\n", gadget_details.port);
		...
		private async AgentSessionId attach_via_remote (uint pid, HashTable<string, Variant> options, RemoteServer server,
				Cancellable? cancellable) throws Error, IOError {
			Fruity.Debug.log ("[FRIDA-HOST-SESSION] Attaching via remote server to PID %u (server flavor: %s)\n", 
				pid, server.flavor.to_string());
			AgentSessionId remote_session_id;
			try {
				Fruity.Debug.log ("[FRIDA-HOST-SESSION] Requesting remote session.attach for PID %u\n", pid);
