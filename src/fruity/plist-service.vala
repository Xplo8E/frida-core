...
namespace Frida.Fruity {
	public sealed class PlistServiceClient : Object {
		...
		public async void close (Cancellable? cancellable) throws IOError {
			Frida.Fruity.Debug.log ("[FRIDA-PLIST-SERVICE] Closing PlistServiceClient\n");
			...
			try {
				...
				Frida.Fruity.Debug.log ("[FRIDA-PLIST-SERVICE] Stream closed successfully\n");
			} catch (IOError e) {
				Frida.Fruity.Debug.log ("[FRIDA-PLIST-SERVICE] Error closing stream: %s\n", e.message);
			}
		}

		public async Plist query (Plist request, Cancellable? cancellable) throws PlistServiceError, IOError {
			Frida.Fruity.Debug.log ("[FRIDA-PLIST-SERVICE] Sending plist query\n");
			write_message (request);
			var response = yield read_message (cancellable);
			Frida.Fruity.Debug.log ("[FRIDA-PLIST-SERVICE] Received plist response\n");
			return response;
		}

		public void write_message (Plist message) {
			uint8[] message_data = message.to_binary ();
			Frida.Fruity.Debug.log ("[FRIDA-PLIST-SERVICE] Writing plist message (%u bytes)\n", message_data.length);
		...
		public async Plist read_message (Cancellable? cancellable) throws PlistServiceError, IOError {
			Frida.Fruity.Debug.log ("[FRIDA-PLIST-SERVICE] Reading single plist message\n");
			var messages = yield read_messages (1, cancellable);
			Frida.Fruity.Debug.log ("[FRIDA-PLIST-SERVICE] Received plist message\n");
			return messages[0];
		}
