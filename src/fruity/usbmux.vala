...
namespace Frida.Fruity {
	public sealed class UsbmuxClient : Object, AsyncInitable {
		...
		public static async UsbmuxClient open (Cancellable? cancellable = null) throws UsbmuxError, IOError {
			Frida.Fruity.Debug.log ("[FRIDA-USBMUX] Opening usbmux client\n");
			var client = new UsbmuxClient ();

			try {
				yield client.init_async (Priority.DEFAULT, cancellable);
				Frida.Fruity.Debug.log ("[FRIDA-USBMUX] Usbmux client opened successfully\n");
			} catch (GLib.Error e) {
				Frida.Fruity.Debug.log ("[FRIDA-USBMUX] Failed to open usbmux client: %s\n", e.message);
				throw_local_error (e);
			}
		...
		private async bool init_async (int io_priority, Cancellable? cancellable) throws UsbmuxError, IOError {
			Frida.Fruity.Debug.log ("[FRIDA-USBMUX] Initializing usbmux client\n");
			...
			if (env_socket_address != null) {
				Frida.Fruity.Debug.log ("[FRIDA-USBMUX] Using environment socket address: %s\n", env_socket_address);
			...
			if (connectable == null) {
#if WINDOWS
				Frida.Fruity.Debug.log ("[FRIDA-USBMUX] Connecting to Windows usbmux on port %d\n", USBMUXD_DEFAULT_SERVER_PORT);
				connectable = new InetSocketAddress (new InetAddress.loopback (SocketFamily.IPV4),
					USBMUXD_DEFAULT_SERVER_PORT);
#else
				Frida.Fruity.Debug.log ("[FRIDA-USBMUX] Connecting to Unix socket: /var/run/usbmuxd\n");
				connectable = new UnixSocketAddress ("/var/run/usbmuxd");
#endif
			} else {
				Frida.Fruity.Debug.log ("[FRIDA-USBMUX] Using default socket address\n");
			}

			try {
				var client = new SocketClient ();
				connection = yield client.connect_async (connectable, cancellable);
				Frida.Fruity.Debug.log ("[FRIDA-USBMUX] Successfully connected to usbmuxd\n");

				var socket = connection.socket;
				if (socket.get_family () != UNIX) {
					Tcp.enable_nodelay (socket);
					Frida.Fruity.Debug.log ("[FRIDA-USBMUX] Enabled TCP_NODELAY on socket\n");
				}
				...
				Frida.Fruity.Debug.log ("[FRIDA-USBMUX] Starting message processing\n");

				process_incoming_messages.begin ();
			} catch (GLib.Error e) {
				Frida.Fruity.Debug.log ("[FRIDA-USBMUX] Failed to connect to usbmuxd: %s\n", e.message);
		...
		public async void close (Cancellable? cancellable = null) throws IOError {
			Frida.Fruity.Debug.log ("[FRIDA-USBMUX] Closing usbmux client\n");
			if (is_processing_messages) {
				is_processing_messages = false;

				io_cancellable.cancel ();
				Frida.Fruity.Debug.log ("[FRIDA-USBMUX] Cancelled I/O operations\n");
		...
		public async void enable_listen_mode (Cancellable? cancellable = null) throws UsbmuxError, IOError {
			Frida.Fruity.Debug.log ("[FRIDA-USBMUX] Enabling listen mode\n");
			assert (is_processing_messages);
			...
			try {
				...
				if (result != ResultCode.SUCCESS) {
					Frida.Fruity.Debug.log ("[FRIDA-USBMUX] Listen mode enable failed with result: %d\n", result);
					throw new UsbmuxError.PROTOCOL ("Unexpected result while trying to enable listen mode: %d", result);
				} else {
					Frida.Fruity.Debug.log ("[FRIDA-USBMUX] Listen mode enabled successfully\n");
				}
			} catch (PlistError e) {
				Frida.Fruity.Debug.log ("[FRIDA-USBMUX] Listen mode enable failed with protocol error: %s\n", e.message);
		...
		public async void connect_to_port (uint device_id, uint16 port, Cancellable? cancellable = null)
			...
			Frida.Fruity.Debug.log ("[FRIDA-USBMUX] Connecting to device %u port %u\n", device_id, port);
			assert (is_processing_messages);
